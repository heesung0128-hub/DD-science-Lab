...
응답 예시 JSON 형식:
{
   ...
   */
    ...
    if (field === "절차_방법") {
      specialInstruction = "\n5. 중요: 현재 영역은 탐구 절차(실험 방법, 통계 분석 절차, 또는 모델 유도 공식 및 수치화 검증 과정)이므로, 각 예문 후보는 반드시 줄바꿈(\\n)을 포함하여 '1. ... \\n2. ... \\n3. ...' 또는 '1단계: ... \\n2단계: ...'와 같이 순차적인 번호가 매겨진 명확한 단계별 설계 리스트 형식으로 도출해 주십시오.";
    }
...
- 탐구 유형: ${type}
- 관련 키워드: ${keywords}
- 탐구 동기: ${report.step_2?.동기 || ""}
- 절차 설계: ${report.step_5?.절차_방법 || ""}

[가이드 및 답변 규칙 - 필수 사항]
1. 현재 작성하려는 영역([${step}단계] - [${field}])에 기입하기에 가장 어울리고 학술적이며 문장력이 뛰어난 완성형 추천 예문 '3가지'를 리스트 형식으로 생성해 주십시오.
2. 예문은 절대로 고정된 템플릿이어선 안 되며, 학생이 앞서 입력한 구체적인 주제(${topic}), 관심사, 유형을 유기적이고 명확하게 반영한 구체적인 맞춤 문장이어야 합니다.
3. 고교 교육과정 범위 내의 정확한 학술 용어를 사용하되, 고등학생 수준에 맞고 즉시 수정해서 활용할 수 있는 현실적이고 매력적인 문장으로 작성하십시오.
4. 응답은 마크다운 코드블록(\`\`\`json) 표식 없이 반드시 아래 구조를 만족하는 순수 JSON 형식으로만 완벽하게 출력하십시오.${specialInstruction}
  ...
  "구체적이고 매력적인 추천 문장 후보 3"
    ...
    if (step === 5) {
      if (field === "절차_방법") {
        if (type === "modeling") {
          return [
            `1단계: '${topic}' 현상을 설명하기 위해 지배 방정식(물리/수학적 공식)을 유도 및 정립한다.\n2단계: 모델에 투입할 핵심 입력 변수와 상수 파라미터(마찰계수, 완치율 등)를 수치화하여 설정한다.\n3단계: 시뮬레이션 코드(또는 소프트웨어)를 제작하고, 입력 변수를 5회 이상 변화시키며 출력 데이터를 연산/추출한다.\n4단계: 도출된 시뮬레이션 데이터를 실제 관측자료와 대조하여 유효성을 최종 검증한다.`,
            `1단계: 현상의 기초 물리량 공식들을 결합하여 '${mainKw}' 요인의 기하학적 형태 변화를 반영한 지배 방정식을 세운다.\n2단계: 변수 간 종속 반응을 모사하는 수치 계산 시뮬레이션 알고리즘을 코딩한다.\n3단계: 다양한 입력값(초기 조건) 조건에서 모델 파라미터를 튜닝하며 출력 데이터 세트를 수집한다.\n4단계: Excel 통계 기법을 적용해 실제 선행 모델과의 상관계수를 산출하고 타당성을 검증한다.`,
            `1단계: '${topic}' 현상 연구를 위해 수치 모델 프레임워크와 지배 수학 공식들을 정의한다.\n2단계: 다양한 물리 상수를 조작하여 모델 파라미터 값으로 세팅한다.\n3단계: 시뮬레이션 실행 후 계산 데이터 로그를 획득하고 그래프로 시각화 가공한다.\n4단계: 유도된 공식의 수치 결과가 교과 내 에너지 보존 법칙/수학적 상관성과 일치하는지 타당성 분석을 실시한다.`
          ];
          ...
          return [
            ...
            `1단계: 분석 독립 변수인 '${mainKw}' 요인 데이터와 결과 종속 변수 간의 분산 및 표준편차를 정립한다.\n2단계: Excel 또는 통계 패키지를 이용해 두 변수 간의 피어슨 상관계수(R)를 유도한다.\n3단계: 유의확률(p-value)을 산출하여 가설의 통계적 유의수준(p < 0.05)을 판정한다.`,
            `1단계: 분석 도구를 활용해 수집된 대규모 시계열 원시 데이터의 이상치를 IQR 기준으로 제거한다.\n2단계: 분석 차원에 따른 각 대상군별 평균값 차이가 존재하는지 t-검정(t-test) 통계 절차를 설계한다.\n3단계: 최종 유도된 통계 분석 표를 기반으로 인과관계와 가설 지지 여부를 판단한다.`
          ];
        } else if (type === "survey") {
          return [
            `1단계: '${topic}'과 관련된 선행 실태 보고서를 기반으로 독립 요인 구분과 설문 측정 항목(5점 리커트 척도)을 설계한다.\n2단계: 교내 동급생 50명 이상을 대상군 표본으로 선정하고 설문조사를 온/오프라인으로 배포 및 회수한다.\n3단계: 수집된 정량 데이터를 통계 소프트웨어에 입력하고 문항별 평균 점수 및 분산을 도출한다.\n4단계: 요인별 집단 간의 만족도나 선호 점수 차이가 유의미한지 분산분석(ANOVA) 절차로 검증한다.`,
            `1단계: 탐구 주제에 어울리는 핵심 대상자 5명을 표집하여 개별 인터뷰(질문지법)를 계획한다.\n2단계: 독립 요인의 차이에 따라 대상자들의 질적 서술 답변 내용을 녹취 및 문자화한다.\n3단계: 핵심 측정 항목에 명시된 주요 범주에 따라 질적 답변들을 부호화(Coding)하여 통계를 낸다.\n4단계: 정성적 답변 분석 결과를 바탕으로 가설에서 추측한 경향성이 타당한지 대조 검증한다.`,
            `1단계: 관찰 대상 지역 또는 현장을 선정하고, 1주일간 매일 특정 시간대에 실태 조사 카드를 작성한다.\n2단계: 현장의 독립 환경 요인들과 측정 항목들의 수치를 수기로 계측하고 기록한다.\n3단계: 관찰 기록 데이터를 스프레드시트에 입력하고 등간격 요인별 추이 그래프로 시각화한다.\n4단계: 실태 조사 데이터를 활용하여 현장 인과 가설의 일치 여부를 통계적으로 검증한다.`
          ];
        }
        
        // experiment or literature (fallback)
        return [
          ...
          `1단계: 수립한 '${topic}' 관련 학술 데이터 및 이론식을 고교 과학 성취기준과 대조하여 분석 단계를 수립한다.\n2단계: 분석 차원을 '시기별 규제 전후'로 설정하고, 비교 대상 국가들의 3개년 평균 데이터를 정렬한다.\n3단계: 비교 분석을 통해 각 대상의 한계점과 오차 범위를 밝혀내고 가설의 유효성을 검증한다.`
        ];
      } else if (field === "도구_자료") {
        return [
