...
0. 학생의 실제 성명(예: 홍길동)은 생활기록부에 기재할 수 없으며, '학생은', '학생이' 같은 주어 대명사 역시 문장마다 반복해 기입할 필요가 전혀 없습니다. 세특 전체 텍스트에서 주어 '학생은'은 아예 생략하거나 최대로쳐도 첫 문장 시작 부분에 딱 한 번만 제한적으로 사용하고, 나머지 문장들에는 주어를 철저히 생략한 서술 형식으로 작성해 주십시오.
...
6. 생활기록부 기재 시 나이스(NEIS) 시스템 입력 오류 방지를 위해, 콜론(:), 등호(=), 단위/물리 기호(%, dB, ℃), 첨자(R²) 등 모든 수학/단위 기호 및 특수문자의 사용을 절대 금지합니다. 퍼센트는 '퍼센트', 데시벨은 '데시벨', 도(온도)는 '도', R²는 '결정계수' 등으로 모든 기호는 반드시 소리 나는 한글로 풀어 서술하십시오. 다만 () 괄호, - 기호, 쌍따옴표나 홑따옴표는 꼭 필요한 경우에만 제한적으로 허용됩니다.

[작성 가이드]
- "적극적인", "탁월한", "우수한" 같은 평가 형용사는 추상적인 인물 평가가 아니라, 구체적인 탐구 행위와 결합할 때만 사용 가능합니다.
  (예: "분석 능력이 우수함" (O), "매우 우수한 학생임" (X))
- 세특의 모든 개별 문장은 마침표(.) 바로 앞에서 반드시 명사형 종결 어미인 "~함", "~임", "~음"으로 끝나야 합니다.
- 절대로 문장 중간이나 끝에 "~한다.", "~했다.", "~하였다.", "~적이다." 등의 평서문 평어체 종결을 사용해 마침표를 찍지 마십시오. 학생의 보고서 인용구에 평서문(~한다)이 포함되어 있다면, 이를 반드시 교사의 관찰형 명사 종결(~함)로 변환하거나, 연결형 어미(~하고,)를 사용해 다음 문장과 자연스럽게 이으십시오.
- 학생 인용구에 포함된 '본 탐구에서는', '본 실험에서는', '본 연구에서는' 등의 조사 표현은 반드시 제거하고 제3자(교사)의 관찰자 시점으로 변환하여 작성하십시오.
...
반드시 마크다운 코드블록 표식 없이 아래의 순수 JSON 구조만 출력하십시오.
{
  ...
  simulateSetuk: function (report, mapping) {
    ...
    if (hasPhysics) {
      ...
      rich = `물리학에 대한 학구열이 매우 뚜렷한 학생으로, '${topic}'을 탐구 과제로 삼아 에어트랙 장비를 활용하여 정밀 구동함. 포토게이트의 시간 해상도 한계를 버니어캘리퍼스로 직접 물리적 교정 세팅하고, 수레 질량비 조건별(일 대 일, 일 대 이) 및 탄성 여부에 따른 총 열 번의 로우 데이터를 엑셀 수식에 대입하여 일차원 역학계 내 운동량 보존 상태를 완벽히 정량화함. 송풍기 공기 불균일에 따른 오차를 스스로 관찰하고 후속으로 이차원 카메라 트래킹 분석까지 모색하는 등 메타인지적 문제해결력과 실험 설계 역량이 탁월함.`;
      ...
      standard = `${courseName} 시간에 배운 상용로그의 실생활 쓰임에 매료되어 '${topic}' 탐구를 설계함. 학교 운동장에 팔십 데시벨 지향성 음원을 설정하고 이의 거듭제곱 수열 거리에 따른 데시벨 수치를 스마트 미터기로 실측함. 음압 소실 비율과 인간 인지 데시벨 척도 간의 상용로그 함수적 규칙성(평균 육점일오 데시벨 감소)을 증명하여 학술적 수학 모델링 능력이 탁월함.`;
